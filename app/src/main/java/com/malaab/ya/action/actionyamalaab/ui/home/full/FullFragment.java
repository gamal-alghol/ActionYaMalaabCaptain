...
public class FullFragment extends BaseFragment implements FullMvpView, OnBottomSheetListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    @Inject
    FullMvpPresenter<FullMvpView> mPresenter;
...
//        mBSheetBook.setOnBottomSheetListener(this);
    ...
    public void onGetCurrentUser(User user) {
        mPresenter.getPlaygrounds(user.address_city, user.latitude, user.longitude);
    }
...
//        mPresenter.listenToFutsalCourtsUpdates(); because it will get the same data again
        ...
        adapter.registerListener(new PlaygroundsAdapter.OnPlaygroundClicked() {
            ...
            public void setOnPlaygroundClicked(Playground playground, int position) {
                playground.isIndividuals = false;
        ...
        if (!StringUtils.isEmpty(playgroundId)) {
            ...
            int position = adapter.getItemPosition(playground);
            playground = adapter.getItem(position);
...
//    }
    ...
    public void OnEventShowPlayground(OnEventShowPlayground event) {
        if (!event.isIndividual()) {
            ...
            if (isPlaygroundsLoaded && !StringUtils.isEmpty(playgroundId)) {
                Playground playground = new Playground();
                playground.playgroundId = playgroundId;
